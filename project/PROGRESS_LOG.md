...
- [🟡] **Admin Role Security**: Đang thực hiện set Admin đầu tiên qua Firestore & Custom Claims.
- [x] **Admin Dashboard Integration**: Khởi tạo layout Admin dựa trên cảm hứng từ Corona React Template bằng Tailwind CSS.

---

## 🔴 3. Các đầu việc tiếp theo (Upcoming Tasks)
### Phase 1: Core Features
- [x] **Authentication**: Login/Register hoàn tất.
...
- [🟡] **Admin Panel**: Xây dựng giao diện quản lý Character Builds (gs/build).
- [ ] **Build Editor**: Implement trình soạn thảo văn bản cho bài viết hướng dẫn nhân vật.
...
| 21/04/2026 | Codex | Setup Option 1 role security: tạo `project/functions` với callable `setUserRole`, thêm frontend service gọi function và helper đọc role từ custom claims trong JWT. |
...
| 21/04/2026 | Gemini | Tích hợp giao diện AdminDashboard.tsx (chuyển về `src/pages/Admin/`) lấy cảm hứng từ Corona Template và cấu hình nested admin routes. |
| 21/04/2026 | Codex | Đọc lại `PROGRESS_LOG.md`, gắn `Navbar` vào `MainLayout` và thêm thanh nav desktop kiểu Genshin (Characters/Builds/Teams/Blog/TCG/Leaderboards/Tools/Tierlist/Wiki). |
